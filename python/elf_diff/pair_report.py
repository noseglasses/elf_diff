...
class PairReport(Report):
    ...
    def generateSimilarSymbolsTableHTML(self):
        ...
        for i in progressbar.progressbar(range(len(self.binary_pair.similar_symbols))):
            ...
            old_highlighted_symbol_name = html.diffStringsSource(
                symbol_pair.old_symbol.name, symbol_pair.new_symbol.name
            )
            new_highlighted_symbol_name = html.diffStringsTarget(
                symbol_pair.old_symbol.name, symbol_pair.new_symbol.name
            )
            if (
                symbol_pair.old_symbol.symbol_type == Symbol.type_data
            ) or symbol_pair.instructions_equal:
                ...
                old_symbol_representation = html.generateSymbolTableEntryLight(
                    old_symbol_name, old_highlighted_symbol_name
                )
                new_symbol_representation = html.generateSymbolTableEntryLight(
                    new_symbol_name, new_highlighted_symbol_name
                )
            ...
            table_html += (
                "<tr><td>%s</td><td>%s<br>%s</td><td><p>%s</p><p>%s</p></td><td><p>%s</p><p>%s</p></td><td>%s</td><td>%s</td><td>%s</td></tr>\n"
                % (
                    table_index,
    ...
    def generatePersistingSymbolDetailsHTML(self):
        ...
        for i in progressbar.progressbar(
            range(len(self.binary_pair.persisting_symbol_names))
        ):
            ...
            if (old_symbol.symbol_type == Symbol.type_data) or (
                new_symbol.symbol_type == Symbol.type_data
            ):
                continue
            ...
            if not old_symbol.__eq__(new_symbol):
                ...
                if old_symbol.size == new_symbol.size:
                    ...
                    size_info = formatMemChange(
                        "size", old_symbol.size, new_symbol.size
                    )
    ...
    def generateDisappearedSymbolDetailsHTML(self):
        ...
        sys.stdout.flush()
        for i in progressbar.progressbar(
            range(len(self.binary_pair.disappeared_symbol_names))
        ):
    ...
    def generateSimilarSymbolDetailsHTML(self):
        ...
        for i in progressbar.progressbar(range(len(self.binary_pair.similar_symbols))):
            ...
            if (
                (symbol_pair.old_symbol.symbol_type == Symbol.type_data)
                or (symbol_pair.new_symbol.symbol_type == Symbol.type_data)
                or symbol_pair.instructions_equal
            ):
                continue
            ...
            old_symbol_representation = html.diffStringsSource(
                symbol_pair.old_symbol.name, symbol_pair.new_symbol.name
            )
            new_symbol_representation = html.diffStringsTarget(
                symbol_pair.old_symbol.name, symbol_pair.new_symbol.name
            )
            ...
            if symbol_pair.old_symbol.size == symbol_pair.new_symbol.size:
                ...
                size_info = formatMemChange(
                    "size", symbol_pair.old_symbol.size, symbol_pair.new_symbol.size
                )

            instruction_similarity_string = ""
            if symbol_pair.instructions_similarity is not None:
                instruction_similarity_string = ", instr. sim.: {:.1f} %".format(
                    symbol_pair.instructions_similarity * 100.0
                )
            ...
            old_title = "<p>Old: %s<br>" % (
                html.generateSymbolDetailsTitleWithRepresentation(
                    old_symbol_name_html, old_symbol_representation
                )
            )
            new_title = "New: %s</p>" % (
                html.generateSymbolDetailsTitleWithRepresentation(
                    new_symbol_name_html, new_symbol_representation
                )
            )
            ...
            symbol_differences = symbol_pair.old_symbol.getDifferencesAsHTML(
                symbol_pair.new_symbol, "   "
            )
    ...
    def configureJinjaKeywords(self, skip_details):
        ...
        [
            ...
            new_symbols_size,
        ] = self.generateNewSymbolsTableHTML()
        [
        ...
        return {
            ...
            "code_size_change_overall": html.formatNumberDelta(
            ...
            "text_size_change_overall": html.formatNumberDelta(
            ...
            "data_size_change_overall": html.formatNumberDelta(
            ...
            "bss_size_change_overall": html.formatNumberDelta(
