...
The following command will generate a html report file named `_elf_diff__pair_report.html` in your current working directory.
```
_elf_diff_ my_old_binary.elf my_new_binary.elf
```
...
A driver file (named `my__elf_diff__driver.yaml`) would then contain the following information:
```
binary_pairs:
...
```
_elf_diff_ --mass_report --driver_file my__elf_diff__driver.yaml
```

This will generate a HTML file `_elf_diff__mass_report.html` in your current working directory.
...
```
_elf_diff_ --pdf_file my_pair_report.pdf my_old_binary.elf my_new_binary.elf
```
...
```
_elf_diff_ --html_file my_pair_report.hmtl my_old_binary.elf my_new_binary.elf
```
or from within a driver file, e.g.
```
...
```
_elf_diff_ --mass_report --pdf_file my_file.pdf ...
```
In `my__elf_diff__driver.yaml`
```
mass_report: True
...
```
_elf_diff_ --bin_dir <path_to_avr_binaries> --bin_prefix "avr-" my_old_binary.elf my_new_binary.elf
```
...
```
_elf_diff_ --driver_template_file my_template.yaml
```
